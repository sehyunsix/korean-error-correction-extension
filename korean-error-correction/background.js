...
// 단축키 명령 처리 (Content Script에서 직접 처리하므로 여기서는 로그만)
// Content Script의 handleShortcut()이 키보드 이벤트를 직접 감지합니다.
chrome.commands.onCommand.addListener(async (command) => {
  ...
  console.log('⏰ 시간:', new Date().toLocaleTimeString());
  console.log('💡 Content Script에서 직접 처리됩니다 (handleShortcut)');
  console.log('='.repeat(80));
  
  // ℹ️ 단축키는 Content Script에서 직접 처리되므로
  // Background에서는 메시지를 전송하지 않습니다.
  // Content Script의 window.addEventListener('keydown', handleShortcut)가
  // Cmd+Shift+E를 감지하고 selection을 즉시 저장하여 처리합니다.
});
